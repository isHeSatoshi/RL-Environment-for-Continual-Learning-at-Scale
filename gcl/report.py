...
def write_results_tex(metrics_path: str, out_tex: str, prefix: str = "") -> Dict[str, Any]:
    ...
    for name, d in learners.items():
        ...
        if isinstance(sc, dict):
            ...
            lines.append(_macro(f"{safe}probespromoted", str(sc.get("probes_promoted", 0))))
            lines.append(_macro(f"{safe}nbhdchecked", str(sc.get("nbhd_checked", 0))))
            lines.append(_macro(f"{safe}nbhdrejected", str(sc.get("nbhd_rejected", 0))))
        vs = d.get("vsr")
