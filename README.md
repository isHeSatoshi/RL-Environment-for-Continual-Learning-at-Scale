...
**New: Self-Certified Continual Learning (SCCL)** — the model certifies its *own* learning targets from the task specification alone (self-spec test bags → discriminative consensus → self-replay veto), so **no gold labels enter the learning loop or the safety gate**. Gold is used only for final evaluation and post-hoc telemetry. The gold-free guarantee is structural (API shape) and enforced by a 64-test proof suite including AST audits and adversarial poisoned-gold environments (`tests/test_sccl.py`).
...
- **SCCL v4 (in-update base anchoring) — plasticity without coverage.** `runs/sccl_v4` (seed 42; pre-registered rule **FAIL** on the arith endpoint): the anchor does not lift the arithmetic holdout (λ=0.1: **0.400** = `sccl`; λ=0.5: **0.200**, worse), but λ=0.1 is the best plasticity arm recorded — ACC **0.688**, BWT **+0.207**, frontier **+0.613** above frozen (**+0.588**), 16 updates with **zero** vetoes/rollbacks. Dissociation: the weak isotropic pull suppresses general drift but tolerates the arith-critical directions; a seed-42 gate audit shows the erosion is concentrated exactly where the gate's recency-window check pool stopped looking (the six certified arith skills were never re-checked once the stream moved on). Gold telemetry (measurement-only) pins the fix on structure, not strictness: 15/17 accepted updates would have been gold-rejected, so a gold gate would have halted learning rather than preserved arith. Determinism: `sccl`/`vsr_nogold` bit-reproduce v3 seed-42 (|Δ|=0.000).
